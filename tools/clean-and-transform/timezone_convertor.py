"""
timezone_convertor.py — reproducible master builder (raw → canonical master).

Reads the raw inputs straight from S3 (data/raw/), builds the hourly master,
converts it to local BG time, and uploads the result to S3 data/processed/.
No local data files are the source of truth — S3 is.

Inputs (S3, data/raw/):
  entsoe_bg/load_actual.csv               + load_forecast_day_ahead.csv
  bulgaria_1day_ahead_forecast_*_UTC.csv  (forecasted weather; latest pick)
  days_off_bg_*.csv                        (latest pick)
      → hourly master (load + ESO forecast + forecasted weather for T + is_day_off)
      → trimmed from 2024-02-17 (start of the weather forecast)
...
# find_key() picks the newest match, so date/hour stamps and sub-folder
# location in the filenames don't matter.
